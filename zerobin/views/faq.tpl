  ...
  <dl>
    ...
    <dd>
        ...
        <p>When somebody wants to read the paste, they will usually click on a link
          with this URL. If the hash containing the key is a part of it, sbin's
    ...
    <dd>
        <p>No, it isn't.</p>
        <p>The goal of sbin is <strong>not</strong> to protect the user and their data
        ...
        <p>Remember that as an user, you should use sbin in the same way as unencrypted and
           insecure pastebins - that is, with caution. The only difference with those is that if
           you decide to host a sbin server, the encryption feature hopefully be used as a defense.
    ...
    <dd>
      <p>Read above.</p>
      <p>sbin is not built, and does not aim, to protect user data - but rather the host.
	 If any user data is compromised, sbin still provides the host with
	 plausible deniability (as they ignore the content of the pastes).</p>
      <p>It would make no sense if the host was to compromise the encryption process
         ...
         installed sbin in the first place, as sbin is here to protect them.</p>
      <p><strong>However, if you want to ensure your data is not read in anyway, you should
		 not use sbin</strong>. Use <a href="http://www.cypherpunks.ca/otr/">OTR</a> for chatting,
    ...
    <dt>How did the idea of sbin emerge?</dt>
    <dd>
      <p>sbin is based on <a href="http://sebsauvage.net/wiki/doku.php?id=php:zerobin">sebsauvage's work</a>.
   ...
   <dt>How can I get sbin?</dt>
   <dd>
      <p>sbin is an open-source project, and the code is hosted on <a href="https://github.com/sametmax/0bin">GitHub</a>.
